    ...
    for dis1_id in all_evaluate_dis[idx:idx+1]:
        ...
        for dis2_id in all_dis:
            if dis2_id not in id2embed:
                continue
            if partition[str(dis1_id)]!=partition[str(dis2_id)]:
                continue
...
global partition
with codecs.open('./partition.json','r',encoding='UTF-8') as f:
    partition=json.load(f)[0]
class2num={}

for a in partition:
    if partition[a] not in class2num:
        class2num[partition[a]]=1
    else:
        class2num[partition[a]]+=1
print(class2num)
#{1: 1202, 2: 1413, 3: 1620, 4: 1706, 5: 1718, 15: 696, 9: 714, 6: 1383,
#   ,7: 153, 0: 958, 13: 315, 14: 111, 8: 8, 10: 135, 11: 177,12: 2073,}
num_rel=2
...
Evaluate...partition(最终的测试集排序平均位置为5.6)
MAP:  0.23774842838854282
P_K:  0.08780000000000036
R_K:  0.42142544122544107

average_position of real acc:  5.618177047952047
'''
