...
public:
    ...
    void run(int n_in,double rate_in,double margin_in,int method_in)
    {
        ...
        cout<<"start load transH"<<endl;

        FILE* f1 = fopen("./entity2vec.txtbern","r");
        for (int i=0; i<entity_num; i++)
        {
            for (int ii=0; ii<n; ii++)
		...
		FILE* f2 = fopen("./relation2vec.txtbern","r");
        for (int i=0; i<relation_num; i++)
        {
            for (int ii=0; ii<n; ii++)
         	...
         	for (int batch = 0; batch<nbatches; batch++)
         	{
         		for (int k=0; k<batchsize; k++)
         		{
					...
					if (rand()%1000<pr)
					{
						...
						//if(ok[make_pair(fb_h[i],fb_r[i])].count(j)<=0) break;}
					...
					else
					{
						while (ok[make_pair(j,fb_r[i])].count(fb_l[i])>0)
							j=rand_max(entity_num);
						//for(j=0;j<entity_num;j++){
                         //       if(ok[make_pair(j,fb_r[i])].count(fb_l[i])<=0) break;}
...
*/
                ...
                for (int i=0; i<relation_num; i++)
                {
                    //cout<<"rel:vec_len "<<vec_len(relation_vec[i])<<endl;
                    for (int ii=0; ii<n; ii++)
                        fprintf(f2,"%.6f ",relation_vec[i][ii]);
                    fprintf(f2,"\n");
                }
                for (int i=0; i<relation_num; i++)
                {
                   // cout<<"A:vec_len "<<vec_len(A[i])<<endl;
                	for (int jj=0; jj<n; jj++)
                		fprintf(f1,"%.6f ",A[i][jj]);
                	fprintf(f1,"\n");
                }
                for (int i=0; i<entity_num; i++)
                {
                    for (int ii=0; ii<n; ii++)
                        fprintf(f3,"%.6f ",entity_vec[i][ii]);
                    fprintf(f3,"\n");
                }
                fclose(f1);
                fclose(f2);
    ...
    void train_kb(int e1_a,int e2_a,int rel_a,int e1_b,int e2_b,int rel_b)
    {
        ...
        if (sum1+margin>sum2)
        {
        	res+=margin+sum1-sum2; //实体对齐，这里res+=margin+a*(sum1-sum2),a为count(r)/T，t为总三元组个数
...
void prepare()
{
	...
	while (fscanf(f_kb,"%s",buf)==1)
    {
        ...
        {
            cout<<"miss relation:"<<endl;
            relation2id[s3] = relation_num;
            id2relation[relation_num]=s3;
            fprintf(f45,"%s\t",buf);
            fprintf(f45,"%d\n",relation_num);
            relation_num++;
        }
